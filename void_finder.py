...
def run(args) -> dict:
    ...
    # CDN classifier (also refreshes live Cloudflare ranges).
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        Console().err(f"unexpected failure: {type(e).__name__}: {e}")
        if args.verbose:
            import traceback
            traceback.print_exc()
        return 1
    ...
    if args.output:
        try:
            ...
            Console().err(f"could not write {args.output}: {e}")

    if args.json:
        print(json.dumps(result, indent=2))

    # Exit code: 0 if an origin was confirmed, 0 otherwise too (informational).
    return 0
